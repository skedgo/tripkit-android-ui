...
@Inject constructor(val context: Context,
  ...
  open fun execute(trip: Trip, tripSegmentId: Long, selectedService: TimetableEntry): Single<TripGroup> {
    return regionService.getRegionByLocationAsync(trip.from)
        .singleOrError()
        .zipWith(tripGroupRepository.getTripSegmentByIdAndTripId(segmentId = tripSegmentId, tripId = trip.uuid()),
             BiFunction<Region, TripSegment, Pair<Region, TripSegment>>{ region, segment -> region to segment })
        .flatMap { (region, segment) ->
          Single.create(WaypointTask(context, configRepository,
