...
class TimetableProvider : ContentProvider() {
    ...
    override fun delete(uri: Uri, sel: String?, selArgs: Array<String>?): Int {
        ...
        try {
            ...
            var rowsAffected = 0
            when (type) {
                SCHEDULED_SERVICES -> rowsAffected =
                    db.delete(DbTables.SCHEDULED_SERVICES.name, sel, selArgs)
            }
    ...
    override fun update(
        ...
        try {
            ...
            when (type) {
                SCHEDULED_SERVICES -> rowsAffected =
                    db.update(DbTables.SCHEDULED_SERVICES.name, values, sel, selArgs)
                SERVICE_ALERT -> rowsAffected =
                    db.update(DbTables.SERVICE_ALERTS.name, values, sel, selArgs)
            }
    ...
    companion object {
        ...
        fun setupConstants(context: Context) {
            AUTHORITY =
                context.packageName + TripKitUI.AUTHORITY_END + TimetableProvider::class.java.simpleName
