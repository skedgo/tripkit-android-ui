...
class GoogleGeocoderLive(private val placeSearchRepository: PlaceSearchRepository) {
    ...
    fun query(
        ...
        neLat: Double,
        neLon: Double
    ): Observable<Place.WithoutLocation> {
        ...
        return placeSearchRepository.searchForPlaces(
            locationName,
            LatLngBounds(LatLng(lowerLeftLat, lowerLeftLon), LatLng(upperRightLat, upperRightLon))
        )
