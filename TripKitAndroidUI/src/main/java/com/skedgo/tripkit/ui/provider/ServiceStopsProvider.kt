...
class ServiceStopsProvider : ContentProvider() {
    ...
    override fun onCreate(): Boolean {
        mDbHelper = DbHelper(
            context!!,
            "tripkit-legacy.db",
            DatabaseMigrator(TripKitDatabase.getInstance(context!!))
        )
        ...
        when (type) {
            STOPS_FOR_SERVICE_ID -> {
                ...
                tablesBuilder.append(" LEFT JOIN ")
                    ...
                    .append(" ON ")
                    .append(DbTables.SCHEDULED_SERVICES).append(".")
                    .append(DbFields.SERVICE_TRIP_ID)
                    .append(" = ")
    ...
    override fun update(
        ...
        try {
            ...
            when (type) {
                ...
                SHAPES -> rowsAffected =
                    db.update(DbTables.SEGMENT_SHAPES.name, values, sel, selArgs)
            }
