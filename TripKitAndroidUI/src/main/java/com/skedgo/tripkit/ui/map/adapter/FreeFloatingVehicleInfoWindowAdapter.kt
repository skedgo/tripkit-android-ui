...
class FreeFloatingVehicleInfoWindowAdapter(private val context: Context) : StopInfoWindowAdapter {
    val view: View by lazy {
        LayoutInflater.from(context)
            .inflate(R.layout.free_floating_vehicle_info_window, null, false)
    }

    @SuppressLint("InflateParams")
    override fun getInfoWindow(marker: Marker): View {
        val entity =
            (marker.tag as FreeFloatingVehiclePOILocation).freeFloatingLocationEntity.vehicle
        ...
        view.findViewById<TextView>(R.id.batteryLevel).text =
            String.format("%d%%", entity.batteryLevel)
