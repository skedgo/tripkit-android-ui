...
class TimetableLoaderFactory {
   ...
   */
  fun createForSegment(context: Context,
                       pairIdentifier: String,
                       sinceSecs: Long): CursorLoader {
    val selection = String.format("%s = ? AND %s > ?", DbFields.PAIR_IDENTIFIER, DbFields.START_TIME)
  ...
  companion object {
    ...
    fun buildQueryParams(stop: ScheduledStop,
                         filter: String?,
                         sinceSecs: Long): TimetableQueryParams {
      ...
      if (stop.isParent) {
        if (!stop.hasChildren()) {
          ...
          selectionBuilder.append(" AND ").append(DbFields.STOP_CODE).append(" IN ($stopCodes)")
        }
      } else {
      ...
      if (!TextUtils.isEmpty(filter)) {
        ...
        if (stop.isParent) {
          ...
          args = arrayOf(sinceSecs.toString(), stop.code, wildcard, wildcard, wildcard, wildcard)
        }
      ...
      return TimetableQueryParams(selection = selection, selectionArgs = args, sortOrder = DbFields.START_TIME.toString() + " ASC")
    }

    fun createForStop(appContext: Context,
                      stop: ScheduledStop,
                      filter: String?,
                      sinceSecs: Long): Loader<Cursor> {
