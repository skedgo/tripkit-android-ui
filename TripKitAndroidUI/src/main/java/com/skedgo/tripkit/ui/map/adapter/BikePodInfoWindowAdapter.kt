...
class BikePodInfoWindowAdapter(private val context: Context) : StopInfoWindowAdapter {
  val view: View by lazy { LayoutInflater.from(context).inflate(R.layout.bike_share_info_window, null, false) }

  @SuppressLint("InflateParams")
  override fun getInfoWindow(marker: Marker): View {
    val bikePodLocationEntity = (marker.tag as BikePodPOILocation).bikePodEntity
    view.findViewById<TextView>(R.id.bikeShareName).text = bikePodLocationEntity.bikePod.operator.name
