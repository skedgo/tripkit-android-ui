...
class DiffTransformer<T, R>(val keySelector: (T) -> String, val createMarkerOptions: (T) -> Single<R>)
  ...
  override fun apply(o: Observable<List<T>>): Observable<Pair<List<Pair<R, T>>, Set<String>>> {
    return o.scan(Triple(emptyList<T>(), emptyList<T>(), emptyList<T>()),
        { oldResult: Triple<List<T>, List<T>, List<T>>, newList: List<T> ->
          ...
          val insertion = newList.map(keySelector).subtract(oldResult.third.map { keySelector.invoke(it) }).map { newSet[it]!! }
          val removal = oldResult.third.map(keySelector).subtract(newList.map { keySelector.invoke(it) }).map { oldSet[it]!! }
